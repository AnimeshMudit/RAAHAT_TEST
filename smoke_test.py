...
def run_test():
    ...
    res = requests.post(f"{BASE_URL}/api/signup", json={"username": email, "password": password})
    ...
    res = requests.post(f"{BASE_URL}/api/verify-otp", json={"email": email, "token": otp})
    if res.status_code == 200:
    ...
    # 3. Login
    print("\n[3] Login Phase")
    res = requests.post(f"{BASE_URL}/api/login", json={"username": email, "password": password})
    if res.status_code == 200:
        print("✅ Login success!")
        user_id = res.json().get("user_id")
    else:
    ...
    res = requests.post(f"{BASE_URL}/api/chat", json={"user_id": user_id, "message": msg1})
    if res.status_code == 200:
    ...
    res = requests.post(f"{BASE_URL}/api/chat", json={"user_id": user_id, "message": msg2})
    if res.status_code == 200:
        response_text = res.json().get("response", "")
...
if __name__ == "__main__":
    try:
        ...
        print("Make sure your FastAPI server is running (e.g. `uvicorn app.api.server:app --reload`) before executing this test.")
        sys.exit(1)
