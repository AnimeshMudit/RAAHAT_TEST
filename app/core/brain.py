...
def get_response(
    ...
    # 2. Second-stage LLM Safety Classifier (Conditional)
    llm_class = "SAFE"
    if not matched_trigger:
        if should_run_safety_classifier(user_message):
...
def get_response_stream(
    ...
    # If this message is classified as HIGH/CRISIS, yield the card at the end
    if bool(matched_trigger) or (llm_class in ("HIGH", "CRISIS")):
        yield "\n\n" + CRISIS_RESOURCE_CARD
